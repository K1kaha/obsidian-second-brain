...
- `adapters/` — platform translation layer. `lib.sh` holds shared parsing helpers. `claude-code/`, `codex-cli/`, `gemini-cli/`, `opencode/`, `kimi-cli/` each ship an `adapter.sh`.
...
Source files in `commands/` use Claude Code's slash-command shape. The Claude Code adapter is an identity copy. The non-Claude adapters (Codex CLI, Gemini CLI, OpenCode) emit a platform-appropriate dispatcher file (`AGENTS.md` or `GEMINI.md`) at the dist root with an **auto-generated routing table** built from each command's `description:` frontmatter, plus the command bodies under `.codex/commands/` (or `.gemini/`, `.opencode/`). The Kimi CLI adapter instead emits a native Kimi Agent Skill under `.kimi/skills/obsidian-second-brain/`, where `SKILL.md` carries the same routing table and the command bodies live in `commands/` beside it. Tool-name references like `Read tool` are rewritten to neutral wording (`read files`) so the instructions still make sense outside Claude Code.
